...
defmodule CrateMovements do
  ...
  def move(piles, from, to, number) do
    to_move = Enum.at(piles, from) |> Enum.take(number)
    new_from = Enum.slice(Enum.at(piles, from), length(to_move)..length(Enum.at(piles,from)))
    piles = List.replace_at(piles, from, new_from)
    piles = List.replace_at(piles, to, to_move ++ Enum.at(piles,to))
    piles
  end
